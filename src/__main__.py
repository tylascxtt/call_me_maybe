...
error and exits non-zero rather than crashing.
    ...
    python -m src [--functions_definition F]
    [--input F] [--output F] [--model ID]
"""
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    """Parse CLI flags (`argv`, or `sys.argv` when `None`)."""
...
def _load_functions(path: Path) -> list[FunctionDefinition]:
    """Read and validate `path` as a JSON array of function definitions."""
    with path.open(encoding="utf-8") as handle:
        return _FUNCTION_DEFINITIONS.validate_python(json.load(handle))


def _load_prompts(path: Path) -> list[str]:
    """Read `path` and return the prompts.

    The file must contain a JSON array of objects, each with a ``prompt`` key.
    ...
    for item in data:
        if not isinstance(item, dict) or "prompt" not in item:
            raise ValueError(
                "every input entry must be an object with a 'prompt' key"
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(
            f"error: input file not found: {exc.filename}",
            file=sys.stderr,
        )
        return 1
    except (json.JSONDecodeError, ValidationError, ValueError) as exc:
        print(
            f"error: could not parse input files: {exc}",
            file=sys.stderr,
        )
        return 1
    ...
    if not functions:
        print(
            "error: no function definitions provided",
            file=sys.stderr,
        )
        return 1
    ...
    def _report(index: int, call: FunctionCall) -> None:
        """Log decoding progress to stderr."""
        print(
            f"[{index + 1}/{len(prompts)}] "
            f"{call.name}({call.parameters})",
            file=sys.stderr,
        )
    ...
    try:
        ...
        print(
            f"error: constrained decoding failed: {exc}",
            file=sys.stderr,
        )
        return 1
    ...
    print(
        f"Wrote {len(results)} function calls "
        f"to {args.output}",
        file=sys.stderr,
    )
    return 0
